...
export function BillPaymentForm({ bill, onSave, onClose }) {
  ...
  return (
    <Card className="w-full max-w-md">
      ...
      <CardContent>
        {bill && <p className="mb-3 text-sm text-gray-600">Balance: {formatCurrency(balance)}</p>}
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <Input type="date" value={form.paidDate} onChange={(e) => setForm((f) => ({ ...f, paidDate: e.target.value }))} required />
          </div>
          <div>
