...
export function BillPaymentForm({ bill, onSave, onClose }) {
  ...
  const balance = (Number(bill?.totalAmount) || 0) - (Number(bill?.paidAmount) || 0);
        ...
        <div>
          ...
          {bill && (
            <p className="text-[13px] font-medium text-slate-500 mt-1">
              Bill: <strong className="text-slate-700">{bill.billNumber || 'Unnumbered'} ({bill.partyName})</strong>
            </p>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          {bill && balance > 0 && (
             <div className="rounded-xl bg-orange-50 border border-orange-100 p-4 flex items-center justify-between shadow-sm">
                ...
                <span className="font-bold text-orange-700 font-mono tracking-tight text-lg">{formatCurrency(balance)}</span>
             </div>
          )}
