...
import { X, Banknote, Calendar, CreditCard, Receipt, FileText, ShieldAlert } from 'lucide-react';
...
export function AssociatePaymentForm({ payment, onSave, onClose }) {
  ...
  const balance = payment ? (Number(payment.agreedAmount) - Number(payment.paidAmount)).toFixed(2) : '0.00';
    ...
    <div className="relative w-full bg-white rounded-3xl shadow-2xl border border-slate-200/60 overflow-hidden">
        ...
        <div>
          ...
          {payment && (
            <p className="text-[13px] font-medium text-slate-500 mt-1">
              Associate: <strong className="text-slate-700">{payment.associate?.name}</strong>
            </p>
          )}
        </div>
        <button type="button" onClick={onClose} className="text-slate-400 hover:text-slate-600 hover:bg-slate-100 p-2 rounded-xl transition-colors self-start">
          <X className="h-5 w-5" />
        </button>
      </div>
      ...
      <div className="max-h-[calc(100vh-8rem)] overflow-y-auto custom-scrollbar">
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          {payment && Number(balance) > 0 && (
             <div className="rounded-xl bg-orange-50 border border-orange-100 p-4 flex items-center justify-between shadow-sm">
                <span className="text-sm font-semibold text-orange-800">Remaining Balance</span>
                <span className="font-bold text-orange-700 font-mono tracking-tight text-lg">₹ {balance}</span>
             </div>
          )}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2 relative">
                <label className="text-sm font-bold text-slate-700">Amount Paid (₹) <span className="text-red-500">*</span></label>
              ...
              <div className="space-y-2 relative">
                <label className="text-sm font-bold text-slate-700">Payment Date <span className="text-red-500">*</span></label>
                <div className="relative">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2 relative">
                <label className="text-sm font-bold text-slate-700 block">Payment Mode <span className="text-red-500">*</span></label>
                <div className="relative">
              ...
              <div className="space-y-2 relative">
                ...
                <div className="relative">
                  <Receipt className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                  <Input 
            ...
            <div className="space-y-2 relative">
              <label className="text-sm font-bold text-slate-700 block">Internal Notes</label>
              <div className="relative">
                ...
                <textarea 
                  ...
                  rows={2} 
                  placeholder="Optional details, remarks, etc."
                />
              </div>
            </div>
          </div>
